							...
							double *fforce, int eflag, double *eng)
{
  double tmp_fc,tmp_fc_d,tmp_exp;
	...
	{
		tmp_fc = 0.5*(1.0 - sin(MY_PI2*(r - param->bigr)*param->bigdinv )); 
		tmp_fc_d =  -(MY_PI4*param->bigdinv) * cos(MY_PI2*(r - param->bigr)*param->bigdinv);
	}
              ...
              double *dri, double *drj, double *drk)
{
  ...
  double dcosdri[3],dcosdrj[3],dcosdrk[3];
	...
	{
		fc = 0.5*(1.0 - sin(MY_PI2*(rik - param->bigr)*param->bigdinv)); 
		dfc =  -(MY_PI4*param->bigdinv) * cos(MY_PI2*(rik - param->bigr)*param->bigdinv);
	}
...
double ters_fc(double r, Param *param)
{
  ...
  return 0.5*(1.0 - sin(MY_PI2*(r - ters_R)*ters_Dinv));
}
...
double ters_fc_d(double r, Param *param)
{
  ...
  if (r < ters_R-ters_D) return 0.0;
  if (r > ters_R+ters_D) return 0.0;
  return -(MY_PI4*ters_Dinv) * cos(MY_PI2*(r - ters_R)*ters_Dinv);
}
                     ...
                     int eflag, double *eng)
{
  double fa,fa_d,bij;
	...
	else
	{
		...
		{
			fc = 0.5*(1.0 - sin(MY_PI2*(r - param->bigr)*param->bigdinv)); 
			dfc =  -(MY_PI4*param->bigdinv) * cos(MY_PI2*(r - param->bigr)*param->bigdinv);
		}
  ...
  if (tmp > param->c1) 
  ...
  else if (tmp > param->c2)
	{
		...
		dbij = param->beta * (-0.5*pow(tmp,-1.5) * (1.0 - (1.0+powerninvhalf)*tmp_nn));
  ...
  else 
	{
		...
		bij = pow(1.0 + tmp_n, -powerninvhalf);
		dbij = -0.5 * pow(1.0+tmp_n, -1.0-powerninvhalf)*tmp_n / zeta_ij;
	}
