void compute_lmff_twolonglist(pair_lmff_param_t *pm)
{
	dma_init();
  ...
  for (ist = my_start; ist < my_end; ist += ISTEP) 
    ...
    pe_get(numneigh+ist,    ni,   sizeof(int)*isz);
  	  ...
  	  int nilp = 0;
			...
			flt_shortneigh[ioff][0] = 0;
			flt_shortneigh[ioff][1] = 0;
			flt_shortneigh[ioff][2] = 0;
			flt_shortneigh[ioff][3] = 0;
  	  ...
  	  fxtmp = fytmp = fztmp = 0.0;
			...
			for (jst = 0; jst < jnum; jst+=JSTEP) 
			{
  	  	  ...
  	  	  rsq = delx*delx + dely*dely + delz*delz;
					...
					if (rsq < cutshortsq) 
					{
						flt_shortneigh[ioff][tot_short] = j;
  	  	    		...
  	  	    		neighshort[numshort] = j;
								neighindex[numshort] = tot_short;
								numshort++;
						}	
						
						/* for ILP */
						flt_shortneigh[ioff][tot_short]	&= ~(1<<30);
					...
					flt_shortneigh[ioff][3] = tot_short;//num;
  	  	}//for-jj
			}//for-jst
		...
		if(isz > 0)
		{
	 		pe_put(l_pm.flt_shortneigh+ist*4,	&flt_shortneigh[0][0],sizeof(int)*isz*4);
	 		dma_syn();
		}
	}//for-ist
