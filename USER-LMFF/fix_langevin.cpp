...
void FixLangevin::post_force(int /*vflag*/)
{
  ...
  if((tstyle != ATOM) && !gjfflag && !tallyflag && (tbiasflag != BIAS) && !rmass && zeroflag)
	{
		my_post_force_000001();
	}
	else if((tstyle != ATOM) && !gjfflag && !tallyflag && (tbiasflag == BIAS) && !rmass && zeroflag)
	{
		my_post_force_000101();
	}
	else
	{
...
void FixLangevin::my_post_force_000001( )
{
...
void FixLangevin::my_post_force_000101( )
{
  double gamma1,gamma2;
...
void FixLangevin::post_force_templated()
{
  ...
  //   subtract sum/count from each atom in group
	...
	//if(comm->me == 0)
	//	printf("(%d,%d,%d,%d,%d,%d)\n", Tp_TSTYLEATOM, Tp_GJF, Tp_TALLY, Tp_BIAS, Tp_RMASS, Tp_ZERO);
