...
PairILPGrapheneHBN::PairILPGrapheneHBN(LAMMPS *lmp) : Pair(lmp)
{
  ...
  tap_flag = 1;
	...
	//form ForceClass;
	force->ilp_nelements	= 0;//added by ping;
  force->ilp_nparams		= 0;//added by ping;
  force->ilp_tap_flag 	= 1;//added by ping;
  force->ilp_cutILPsq		= NULL;//added by ping;
  force->ilp_map				= NULL;//added by ping;
	force->ilp_elem2param = NULL;//added by ping;
	force->ilp_params			= NULL;//added by ping;
...
PairILPGrapheneHBN::~PairILPGrapheneHBN()
{
  ...
  if (allocated) {
    ...
    memory->destroy(force->ilp_cutsq);//added by ping;
    memory->destroy(cut);
  ...
  memory->destroy(elem2param);
	memory->destroy(force->ilp_params);//added by ping;
  memory->destroy(force->ilp_elem2param);//added by ping;
  
	if (allocated) 
	{
		delete [] map;
		delete [] force->ilp_map;//added by ping;
	}
}
...
void PairILPGrapheneHBN::allocate()
{
  ...
  memory->create(force->ilp_cutsq,n+1,n+1,"pair:ilp_cutsq");//added by ping;
  memory->create(cut,n+1,n+1,"pair:cut");
  memory->create(offset,n+1,n+1,"pair:offset");
  map = new int[atom->ntypes+1];
  force->ilp_map = new int[atom->ntypes+1];//added by ping;
...
void PairILPGrapheneHBN::settings(int narg, char **arg)
{
  ...
  if (narg == 2) 
	{
		tap_flag = force->numeric(FLERR,arg[1]);
		force->ilp_tap_flag = tap_flag;//added by ping;
	}
...
void PairILPGrapheneHBN::coeff(int narg, char **arg)
{
  ...
  for (i = 3; i < narg; i++) 
    ...
    if (strcmp(arg[i],"NULL") == 0) 
		{
      map[i-2] = -1;
      force->ilp_map[i-2] = -1;//added by ping;
      continue;
    }
    ...
    map[i-2] = j;
    force->ilp_map[i-2] = j;//added by ping;
	...
	force->ilp_nelements = nelements;//added by ping;
...
double PairILPGrapheneHBN::init_one(int i, int j)
{
  ...
  offset[j][i] = offset[i][j];
	...
	force->ilp_cutsq[i][j] = force->ilp_cutsq[j][i] = cut[i][j]*cut[i][j];//added by ping;
...
void PairILPGrapheneHBN::read_file(char *filename)
{
  ...
  memory->sfree(params);
  memory->sfree(force->ilp_params);//added by ping;
  ...
  while (1) {
    ...
    if (nparams == maxparam) 
      ...
      force->ilp_params = (Param_ilp_str *) memory->srealloc(force->ilp_params,maxparam*sizeof(Param_ilp_str),"pair:params");//added by ping;
    }

		//added by ping;
		force->ilp_params[nparams].ielement = params[nparams].ielement = ielement;
   	force->ilp_params[nparams].jelement = params[nparams].jelement = jelement;
   	force->ilp_params[nparams].z0       = params[nparams].z0       = atof(words[2]);
   	force->ilp_params[nparams].alpha    = params[nparams].alpha    = atof(words[3]);
   	force->ilp_params[nparams].delta    = params[nparams].delta    = atof(words[4]);
   	force->ilp_params[nparams].epsilon  = params[nparams].epsilon  = atof(words[5]);
   	force->ilp_params[nparams].C        = params[nparams].C        = atof(words[6]);
   	force->ilp_params[nparams].d        = params[nparams].d        = atof(words[7]);
   	force->ilp_params[nparams].sR       = params[nparams].sR       = atof(words[8]);
   	force->ilp_params[nparams].reff     = params[nparams].reff     = atof(words[9]);
   	force->ilp_params[nparams].C6       = params[nparams].C6       = atof(words[10]);
    
		// S provides a convenient scaling of all energies
		force->ilp_params[nparams].S    = params[nparams].S        = atof(words[11]);
    force->ilp_params[nparams].rcut = params[nparams].rcut     = atof(words[12]);
    ...
    params[nparams].seff					= params[nparams].sR * params[nparams].reff;
		...
		force->ilp_params[nparams].C					= params[nparams].C;
    force->ilp_params[nparams].C6				= params[nparams].C6;
    force->ilp_params[nparams].epsilon 	= params[nparams].epsilon;
		force->ilp_params[nparams].delta2inv = params[nparams].delta2inv;
    force->ilp_params[nparams].lambda		= params[nparams].lambda;
    force->ilp_params[nparams].seff			= params[nparams].seff;
    force->ilp_params[nparams].seffinv	= 1 / params[nparams].seff;

    nparams++;
  }

	force->ilp_nparams = nparams;//added by ping;

  memory->destroy(elem2param);
	memory->destroy(cutILPsq);
	memory->destroy(force->ilp_cutILPsq);//added by ping;
  memory->create(elem2param,nelements,nelements,"pair:elem2param");
  memory->create(force->ilp_elem2param,nelements,nelements,"pair:elem2param");//added by ping;
	memory->create(cutILPsq,nelements,nelements,"pair:cutILPsq");
  memory->create(force->ilp_cutILPsq,nelements,nelements,"pair:ilp_cutILPsq");//added by ping;
    ...
    for (j = 0; j < nelements; j++) 
      ...
      if (n < 0) error->all(FLERR,"Potential file is missing an entry");
			...
			elem2param[i][j] = n;
      force->ilp_elem2param[i][j] = n;//added by ping;
      cutILPsq[i][j] = params[n].rcut*params[n].rcut;
      force->ilp_cutILPsq[i][j] = cutILPsq[i][j];//added by ping;
    }
  }
  delete [] words;
}
...
inline void ev_tally_buffer_c_test(int i, int j, double *ei, double *vi, 
                            double *ej, double *vj, 
                            int nlocal, int newton_pair,
                            double evdwl, double ecoul,
                            double fx, double fy, double fz,
                            double delx, double dely, double delz, 
                            int eflag_either, int vflag_either,
                            int eflag_atom, int eflag_global,
                            int vflag_atom, int vflag_global,
                            double *eng_vdwl, double *eng_coul,
                            double (*vatom)[6], double *eatom, double *virial)
{
...
void PairILPGrapheneHBN::compute_ilp(int eflag, int vflag)
{
  ev_init(eflag,vflag);
	pvector[0] = pvector[1] = 0.0;
