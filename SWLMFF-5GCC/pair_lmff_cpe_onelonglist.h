	      ...
	      for (jj = jst; jj < jed; jj++) 
		{
		  ...
		  //lwpf_start(PART1);
		      ...
		      if (vdwflag) 
			{
			  ...
			  r8inv = r6inv*r2inv;
			  //lwpf_start(ATT);
			  //TSvdw = f1 + exp(-p->d*(r*p->seffinv - f1));
			  TSvdw = f1 + xexp(-p->d*(r*p->seffinv - f1));
			  TSvdwinv = f1 / TSvdw;
			  ...
			  // derivatives
			  fpair = -f6*p->C6*r8inv*TSvdwinv + p->C6*p->d*p->seffinv*(TSvdw-f1)*TSvdw2inv*r8inv*r;
		      ...
		      frho1 = exp1*p->C;
		      Erep = f0_5*p->epsilon + frho1;
		      Vilp = exp0*Erep;

		      // derivatives
		      fpair  = p->lambda*exp0*rinv*Erep;
		      fpair1 = f2*exp0*frho1*p->delta2inv;
		      fsum = fpair + fpair1;
		      //lwpf_stop(PART2);		
