...
{
	GPTLstart("forward comm compute");
  int i,irecv,n,nsend,nrecv;
...
{
	GPTLstart("reverse comm compute");
  int i,irecv,n,nsend,nrecv;
...
{
	GPTLstart("box drop tiled");
  box_drop_tiled_recurse(lo,hi,0,nprocs-1,indexme);
	GPTLstop("box drop tiled");
}
...
void CommTiled::grow_swap_send(int i, int n, int nold)
{
  ...
  for (int j = 0; j < n; j++) {
	...
	GPTLstop("grow swap send");
}

void CommTiled::grow_swap_recv(int i, int n)
{
	GPTLstart("grow swap recv");
  delete [] recvproc[i];
